...
router.post("/login", bruteforce.prevent, async (req,res) =>{
    ...
    try{
        ...
        if(!passwordMatch){
            ...
            const token = jwt.sign({username:req.body.username,password:req.body.password},"this_secret_should_be_no_longer_than_it_is",{expiresIn:"1h"})
            res.status(200).json({message:"Authentication Successful", token: token, name: req.body.name});
            console.log("your new token is", token)
        }
    
    } catch(error){
        console.error("Login Error:", errror);
